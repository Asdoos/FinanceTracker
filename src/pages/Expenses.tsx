              ...
              return (
                <div key={key} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
                  ...
                  >
                    ...
                    <span className="font-semibold text-white text-sm">{cat?.name ?? "Ohne Kategorie"}</span>
                    <span className="text-gray-500 text-xs">{items.length}</span>
                    <span className="ml-auto text-sm font-medium text-white mr-1">
                  ...
                  {!isCollapsed && items.map((e) => (
                    <div
                      key={e.id}
                      className={`border-t border-gray-800/50 px-4 py-3 ${!e.isActive ? "opacity-40" : ""}`}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0 flex-1">
                          <p className="font-medium text-white text-sm truncate">{e.label}</p>
                          ...
                          {e.endDate && (() => {
                            ...
                            return (
                              <p className={`text-xs mt-0.5 ${expired ? "text-red-400" : "text-gray-500"}`}>
                          ...
                          <div className="flex items-center gap-2 mt-1 flex-wrap">
                            ...
                            <span className="text-xs text-gray-500">{pct(e.shareOfTotal)}</span>
                          </div>
                        </div>
                        <div className="flex flex-col items-end flex-shrink-0 gap-1">
                          <span className="text-white font-medium text-sm">{eur(e.monthlyAmount)}/Mo</span>
                          <span className="text-gray-500 text-xs">
                            {eur(e.amount)}{e.type === "annual" ? "/Jahr" : "/Mo"}
                          </span>
